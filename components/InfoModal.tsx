...
const InfoModal: React.FC<InfoModalProps> = ({ movie, initialTime = 0, onClose, onPlay, trailerId }) => {
    const { myList, toggleList, updateVideoState } = useGlobalContext();
    ...
    useEffect(() => {
        if (movie) {
            ...
            // 2. Load Video (Prioritize passed ID or Fetch)
            if (trailerId) {
                ...
                <div className="relative h-[250px] sm:h-[350px] md:h-[480px] w-full bg-black group overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-6 md:p-12 space-y-4 md:space-y-6 z-20 pointer-events-auto">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            >
                                ...
                                {resumeContext && mediaType === 'tv' ? `Resume S${resumeContext.season} E${resumeContext.episode}` : t('hero.play')}
                            </button>
                            <button
                    ...
                    {isPlayingTrailer && (
                        <button
                            ...
                            className="absolute bottom-4 right-4 z-30 w-8 h-8 rounded-full bg-black/50 border border-white/30 flex items-center justify-center hover:bg-white/20 transition pointer-events-auto"
                        >
